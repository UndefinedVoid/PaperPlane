...
wsServer.on('request', function(request) {
    ...
    connection.on('message', function(message) {
        if (message.type === 'utf8') { // accept only text
            console.log((new Date())+' Client Sent Login Request : '+message.utf8Data);
...
mongoDb.connect(dbUrl, function(err, db) {
 ...
 dbo.collection("users").findOne({UserName:user.UserName}, function(err, result) {
    if (err) throw err;
	if (result!=null) {
		console.log("User : " + result.UserName+" not exists.");
	     connection.sendUTF(JSON.stringify( { type: 'already'} ));
	  
	    }
	else {console.log("User Logged In : " + a);
            connection.sendUTF(JSON.stringify( { type: 'logged', data:a} ));
